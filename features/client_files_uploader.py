...
class ClientOneDriveUploader:
    ...
    def create_folder(self, candidate, uploaded_file):
        if uploaded_file is not None and candidate and self.user_id:
            ...
            url = f"https://graph.microsoft.com/v1.0/users/{self.user_id}/drive/root:/Agreements:/children"
    ...
    def upload_file(self, candidate, uploaded_file):
        ...
        if response.status_code in [200, 201]:
            ...
            if link_response.status_code in (200, 201):
                ...
                self.run_sql("Insert into link_mapping (filename, urllinks, company_name) values (%s, %s, %s)",
                             (uploaded_file.name, web_url, candidate))
                return web_url
        return 'NaN'
